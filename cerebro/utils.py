...
def rmspe_loss(y_pred, y_true, epsilon=1e-8, use_close=True):
    """
...
def mape_loss(y_pred, y_true, epsilon=1e-8, use_close=True):
    """
...
def rmse_loss(y_pred, y_true, use_close=True):
    """
    Root Mean Squared Percentage Error Loss
    Works with log-transformed data (applies exp to get original scale)
    """
...
def mae_loss(y_pred, y_true, use_close=True):
    """
    Mean Absolute Percentage Error Loss
    Works with log-transformed data (applies exp to get original scale)
    """
